...
const EditProduct: React.FC = () => {
  ...
  const handleAddVariation = () => {
    setVariations((prev) => [...prev, Date.now()]);
  };

  const handleRemoveVariation = (id: number) => {
    setVariations((prev) => prev.filter((v) => v !== id));
  };
              ...
              <Col xs={24} lg={16}>
                <div className="form-section">
                  ...
                  <Row gutter={16}>
                    ...
                    <Col span={12}><Form.Item name="details" label="Product Details"><QuillEditor /></Form.Item></Col>
                    ...
                    {/* Static variation form */}
                    <div className='repeat-form'>
                      <Row gutter={16}>
                        <Col span={12}><Form.Item name="length" label="Length"><Input type='number' defaultValue={0} className="custom-input" /></Form.Item></Col>
                        <Col span={12}><Form.Item name="stock" label="Product stock"><Input type='number' defaultValue={1} placeholder="Enter Stock" className="custom-input" /></Form.Item></Col>
                        <Col span={12}><Form.Item name="weight" label="Weight"><Input type='number' defaultValue={0} placeholder="Enter Weight" className="custom-input" /></Form.Item></Col>
                        <Col span={12}><Form.Item name="color" label="Color"><Input defaultValue={""} placeholder="Enter color" className="custom-input" /></Form.Item></Col>
                      </Row>
                    </div>

                    {/* Dynamic Variation Forms */}
                    {variations.map((id, index) => (
                      <div className='repeat-form' key={id} style={{ border: '1px solid #ccc', padding: 16, borderRadius: 8, marginBottom: 16 }}>
                        <Row gutter={16}>
                          <Col span={12}><Form.Item name={["variations", id, "length"]} label="Length"><Input type='number' defaultValue={0} className="custom-input" /></Form.Item></Col>
                          <Col span={12}><Form.Item name={["variations", id, "stock"]} label="Product Stock"><Input type='number' defaultValue={1} className="custom-input" placeholder="Enter Stock" /></Form.Item></Col>
                          <Col span={12}><Form.Item name={["variations", id, "weight"]} label="Weight"><Input type='number' defaultValue={0} className="custom-input" placeholder="Enter Weight" /></Form.Item></Col>
                          <Col span={12}><Form.Item name={["variations", id, "color"]} label="Color"><Input defaultValue="" className="custom-input" placeholder="Enter color" /></Form.Item></Col>
                          <Col span={24} style={{ textAlign: 'right' }}>
                            <Button icon={<MinusCircleOutlined />} danger onClick={() => handleRemoveVariation(id)} type="dashed">
                              Remove Variation
                            </Button>
                          </Col>
                        </Row>
                    ...
                    <Col span={24} style={{ textAlign: 'left' }}>
                      <Button icon={<PlusOutlined />} onClick={handleAddVariation} type="dashed">
                        Add Variation
                      </Button>
                    </Col>
                  </Row>
