...
const FuelPrice: React.FC = () => {
  ...
  const columns = [
    ...
    {
      title: 'Today Fuel Price',
      dataIndex: 'todayFuelPrice',
      key: 'todayFuelPrice',
    },
    {
      title: 'Creation Date',
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        const menu = (
          <Menu>
            <Menu.Item key="edit" icon={<Edit size={16} />} onClick={() => navigate('/add-fuel-prices')}>
              Edit
            </Menu.Item>
  ...
  const filteredData = userData.filter(user =>
    user.todayFuelPrice.toLowerCase().includes(searchText.toLowerCase()) ||
    user.creationDate.toLowerCase().includes(searchText.toLowerCase())
  );

  return (
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title">Manage Fuel</h1>
        <Button type="primary" onClick={() => navigate('/add-fuel-prices')}>
        SHOW FUEL PRICE
        </Button>
      </div>
